...
!============================================================
    ...
    Real :: PI,PI05,PI2
    Real(8) :: CMAS,C22,OG,KZZ,GM
    Real(8) :: XP(MX),YP(MX),XQ(NQ),YQ(NQ)
    Real(8) :: VN(3,NP)
...
300    CONTINUE
600 FORMAT(/15X,'NONDIMENSIONAL MASS------- S/(B/2)**2=',F8.5,&
...
!============================================================
    ...
    IF(R>25.0D0)  GO TO 30
    IF(X>0.0D0.AND.R>8.0D0)  GO TO 20
    IF(X<=0.0D0.AND.Y>10.0D0) GO TO 20
    ...
    SB=-R
      ...
      EI=EI-SB*DSIN(CN)
      IF(N==100)  GO TO 1
      IF(EI==0.0D0)  GO TO 10
      IF(DABS(SB/EI)<=1.0D-8) GO TO 10
        GO TO 100
 10   IF(DABS(SB/ER)<=1.0D-8) GO TO 1
100   CONTINUE
  1 CC=DEXP(X)*DCOS(Y)
    SS=DEXP(X)*DSIN(Y)
    EC=CC*ER-SS*EI
 ...
 30 OLD=-1.0D0/R
    ...
    EXS=OLD*DSIN(C)
      DO 300 N=2,100
        ...
        IF(EXS==0.0D0) GO TO 31
        IF(DABS(NEW/EXS)<=1.0D-8) GO TO 31
        GO TO 32
 31   IF(EXC==0.0D0) GO TO 32
      IF(DABS(NEW/EXC)<=1.0D-8) GO TO 33
 32   IF(DABS(OLD)<DABS(NEW))  GO TO 33
      OLD=NEW
